...
class SudokuSolver:
    ...
    def apply_hidden_pair(self, puzzle: np.ndarray) -> bool:
        ...
        for row in range(9):
            progress |= self.find_hidden_pairs_in_unit(
                [(row, col) for col in range(9) if (row, col) in self.candidates], 'row'
            )
        
        for col in range(9):
            progress |= self.find_hidden_pairs_in_unit(
                [(row, col) for row in range(9) if (row, col) in self.candidates], 'col'
            )
    ...
    def apply_naked_triple(self, puzzle: np.ndarray) -> bool:
        ...
        for row in range(9):
            progress |= self.find_naked_triples_in_unit(
                [(row, col) for col in range(9) if (row, col) in self.candidates]
            )
        
        # Check columns
        for col in range(9):
            progress |= self.find_naked_triples_in_unit(
                [(row, col) for row in range(9) if (row, col) in self.candidates]
            )
    ...
    def apply_hidden_single_row(self, puzzle: np.ndarray) -> bool:
        ...
        for row in range(9):
            ...
            empty_cells = [(row, col) for col in range(9) if (row, col) in self.candidates]
    ...
    def apply_hidden_single_column(self, puzzle: np.ndarray) -> bool:
        ...
        for col in range(9):
            ...
            empty_cells = [(row, col) for row in range(9) if (row, col) in self.candidates]
    ...
    def apply_full_house_row(self, puzzle: np.ndarray) -> bool:
        ...
        progress = False
        for row in range(9):
            empty_cells = [col for col in range(9) if puzzle[row, col] == 0]
            if len(empty_cells) == 1:
                col = empty_cells[0]
                # Find missing value
                present_values = set(puzzle[row, :]) - {0}
                missing_value = (set(range(1, 10)) - present_values).pop()
    ...
    def apply_full_house_column(self, puzzle: np.ndarray) -> bool:
        ...
        progress = False
        for col in range(9):
            empty_cells = [row for row in range(9) if puzzle[row, col] == 0]
            if len(empty_cells) == 1:
                row = empty_cells[0]
                # Find missing value
                present_values = set(puzzle[:, col]) - {0}
                missing_value = (set(range(1, 10)) - present_values).pop()
                puzzle[row, col] = missing_value
                self.update_candidates_after_assignment(puzzle, row, col, missing_value)
                progress = True
    ...
    def apply_full_house_box(self, puzzle: np.ndarray) -> bool:
        ...
        for box_row in range(0, 9, 3):
            for box_col in range(0, 9, 3):
                ...
                empty_positions = [(r, c) for r in range(3) for c in range(3) if box[r, c] == 0]
                
                if len(empty_positions) == 1:
                    r, c = empty_positions[0]
                    actual_row, actual_col = box_row + r, box_col + c
                    # Find missing value
                    present_values = set(box.flatten()) - {0}
                    missing_value = (set(range(1, 10)) - present_values).pop()
    ...
    def validate_solution(self, puzzle: np.ndarray) -> bool:
        ...
        # Check rows
        for row in range(9):
            if set(puzzle[row, :]) != set(range(1, 10)):
                return False
        
        # Check columns
        for col in range(9):
            if set(puzzle[:, col]) != set(range(1, 10)):
                return False
